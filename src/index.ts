...
const setResources = (resources: { code: string; id: string }[]) => {
    ...
    const removed = list.filter((i) => !resources || !resources.find((r) => r.code === i.code));
...
export const withStyleResources = makeDecorator({
    name: 'withStyleResources',
    ...
    wrapper: (getStory, context, { options, parameters }) => {
        ...
        if (!Array.isArray(storyOptions) && !Array.isArray(storyOptions.cssresources)) {
            throw new Error('The `addon-styles` parameter needs to be an Array');
        }
